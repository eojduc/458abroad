...
@Controller
public record StudentsController(StudentRepository studentRepository, UserService userService,
  ...
  public String getStudents(Model model, HttpSession session) {
    ...
    students.stream().findFirst().ifPresent(student -> userService.setUser(session, student));
